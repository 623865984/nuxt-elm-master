<template>
  <div class="shopcart">
    <div 
      ...
      <router-link :to="userInfo&&userInfo.user_id? '/order/payment' : '/login'">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      setOrderInfo(){
        if(this.totalPrice >= this.minPrice){
          this.$store.commit('setSeletFoods',this.selectFoods);
          this.$store.commit('setDeliveryPrice',this.deliveryPrice);
        }
      },
      hideList() {
        this.fold = true;
